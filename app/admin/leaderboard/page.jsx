  ...
  const handleTogglePublish = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        ...
        {
          ...currentSettings,
          is_leaderboard_public: newStatus
        },
        { headers: { Authorization: `Bearer ${newToken}` } }
      );
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-10 font-sans text-slate-800">
      ...
      <div className="max-w-7xl mx-auto mb-10 text-center md:text-left">
        <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 mb-3 items-center text-center tracking-tight italic uppercase">Leaderboard</h1>
        <p className="text-slate-500 text-sm md:text-lg text-center font-medium">Recognizing the top change-makers in the SDG challenge.</p>
        ...
        <div className="flex bg-slate-100 p-1 rounded-2xl w-full lg:w-auto">
          {['round_1', 'round_2', 'round_3'].map((round) => (
            <button
              ...
              className={`flex-1 lg:flex-none px-5 md:px-8 py-2.5 rounded-xl text-xs font-black transition-all duration-300 ${activeRound === round ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
            >
      ...
      {loading ? (
        ...
        <>
          ...
          {/* Table List */}
          <div className="max-w-7xl mx-auto bg-white rounded-[2rem] shadow-xl border border-slate-100 mb-10">
            ...
            <div className="w-full overflow-x-auto p-5 ">
              <table className="w-full min-w-[640px] text-left ">
                ...
                {/* Header */}
                <thead>
                  <tr className="bg-slate-50/50 border-b border-slate-100 text-[10px] md:text-xs">
                    <th className="p-3 md:p-6 font-black text-slate-400 uppercase tracking-widest">Rank</th>
                    <th className="p-3 md:p-6 font-black text-slate-400 uppercase tracking-widest">Participant</th>
                    <th className="p-3 md:p-6 font-black text-slate-400 uppercase tracking-widest">SDG</th>
                    <th className="p-3 md:p-6 font-black text-slate-400 uppercase tracking-widest text-right">Performance</th>
                  </tr>
                </thead>
                  ...
                  {listData.map((student) => (
                    ...
                    >
                      ...
                      <td className="p-3 md:p-6">
                        <span className="text-base md:text-xl font-black text-slate-300 group-hover:text-indigo-600">
                          #{student.rank}
                        </span>
                      </td>
                      ...
                      <td className="p-3 md:p-6">
                        <div className="flex items-center gap-3 md:gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-[9px] md:text-[10px] text-slate-400 font-black uppercase">
                              {student.institution}
                            </p>
                          </div>
                        </div>
                      ...
                      {/* SDG */}
                      <td className="p-3 md:p-6">
                        <span className="px-2 md:px-3 py-1 rounded-lg bg-indigo-100 text-indigo-700 text-[9px] md:text-[10px] font-black uppercase">
                          SDG {student.sdg}
                        </span>
                      </td>

                      {/* Score */}
                      <td className="p-3 md:p-6 text-right font-black text-slate-900">
                        {student.score}
                        <small className="text-slate-400 text-[9px] md:text-[10px] ml-1">
                          PTS
                        </small>
                      </td>
                    </tr>
