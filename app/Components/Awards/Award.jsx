...
import { 
  ...
  FaHandsHelping, FaLightbulb, FaComments, 
  FaChalkboardTeacher, FaProjectDiagram, FaHeartbeat, 
  FaTrophy, FaTicketAlt, FaBriefcase, FaAward
} from "react-icons/fa";
...
export default function AwardsSection() {
  ...
  return (
    <section className="relative w-full py-16 md:py-24 px-4 overflow-hidden">
      ...
      {/* --- Background Wrapper (Fixed only for this section) --- */}
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none" style={{ clipPath: 'inset(0 0 0 0)' }}>
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
           <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-md border border-white/20 text-white text-[10px] md:text-xs px-4 py-1.5 rounded-full mb-4">
             <FaAward className="text-Primary" /> 17 finalists who will receive awards
           </div>
        ...
        <div className="grid  grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-20">
          {awards.map((item, i) => <GlassCard key={i} {...item} />)}
        </div>

        {/* --- Scholarships Grid --- */}
        <div className="text-center mb-10">
           <h2 className="text-3xl md:text-5xl font-bold text-white tracking-tight">
             Scholarships & Sponsorships <span className="text-Primary font-medium">(Season One)</span>
           </h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-20">
          {scholarships.map((item, i) => <GlassCard key={i} {...item} />)}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto mb-16">
          {activities.map((item, i) => <WideGlassCard key={i} {...item} />)}
        </div>
